    ...
    updateTrack: function() {
        ...
        if (isCurrentAlbum) {
            this.$('li').each(function(index, el) {
                isCurrentTrack = (this.player.get('currentTrackIndex') === index);
                $('el').toggleClass('current', isCurrentTrack);
            });
        }
    }
});
...
Tunes.PlaylistView = Backbone.View.extend({
    ...
    updateState: function() {
        this.updateTrack();
        this.$('button.play').toggle(this.player.isPlaying());
        this.$('button.pause').toggle(this.player.isStopped());
    },
